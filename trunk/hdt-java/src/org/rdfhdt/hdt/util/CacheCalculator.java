 ...
 * for on-disc implementations of ModifiableTriples and ModifiableDictionaries
 * 
 * @author Eugen Rozic
 */
public class CacheCalculator {
	...
	/**
	 * This method gets the "tempTriples.cache" property form the specifications. If the property is
	 * unset or set to "auto" then this method will calculate the amount of cache for a
	 * disc-based implementation of ModifiableTriples.
	 * 
	 * The calculation is done with the following formula:
	 * Xmx - (2*expectedCompression*sizeOfRDF) - tempDictionary.Cache.
	 * 
	 * if the type of the dictionary is "on-disc"(much more likely) then if the dictionary cache
	...
	public static long getTriplesCache(HDTSpecification specs){
		...
		if (DictionaryFactory.MOD_DICT_TYPE_ON_DISC.equals(dictionaryType)){
			...
			//TODO what is the formula if the dictionary is in_mem and triples on_disc (little likely combination)
	 ...
	 * unset or set to "auto" then this method will calculate the amount of cache for a
	 * disc-based implementation of ModifiableDictionary.
	 * 
	 * The calculation is done with the following formula:
	 * Xmx - (2*expectedCompression*sizeOfRDF) - tempTriples.Cache.
	 * 
	 * If the type of the triples is "on-disc" then if the triples cache
	 * is already set it is just subtracted (as seen in the formula), and if it's not already
	...
	public static long getDictionaryCache(HDTSpecification specs){
		...
		if (TriplesFactory.MOD_TRIPLES_TYPE_ON_DISC.equals(triplesType)){
			bytes = Xmx - (long)(2*compression*sizeOfRDF);
			if (triplesCache!=-1){
				//if dictionary cache already calculated/set then just subtract
				bytes -= triplesCache;
