    ...
    extends CmdARQ
{
	...
	{
		FusekiHDTCmd inner = new FusekiHDTCmd(argv);
		inner.process();
	...
	protected FusekiHDTCmd(String... argv)
	{
		super(argv);
		...
		// Control the order!
		add(argHdt, "--hdt",
		    "use an existing HDT backed dataset for the server");
...
//            add(argRealm, "--realm=REALM", "Realm name");
	...
	protected void processModulesAndArgs()
	{
		...
		serverConfig.allowUpdate = false;
		...
		if (contains(argHdt))
		{
			...
			try
			{
				...
				serverConfig.dsg = DatasetGraphFactory.wrap(graph);
	...
	protected void exec()
	{
		try
		{
			...
			} catch (FusekiException ex)
			{
				...
				{
					Fuseki.serverLog
					    .error("Failed to start server: " + ex.getCause().getMessage() + ": port=" + serverConfig.port);
					System.exit(1);
				}
				throw ex;
			} catch (Exception ex)
